...
class SIPUAWebSocketImpl {
  ...
  void connect(
    ...
    try {
      if (webSocketSettings.allowBadCertificate || _url.startsWith('wss://')) {
  ...
  Future<WebSocket> _connectForBadCertificate(
    ...
    try {
      Random r = Random();
      // base64 key must NOT be lowercased — it's a case-sensitive opaque value
      ...
      webSocketSettings.extraHeaders.forEach((String k, dynamic value) {
        request.headers.add(k, value, preserveHeaderCase: true);
      });
