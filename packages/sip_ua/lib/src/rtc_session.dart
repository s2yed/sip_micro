...
String _fixSdp(String? sdp, String wanIp) {
  if (sdp == null) return '';

  print('=== _fixSdp INPUT START ===');
  sdp.split('\n').forEach((l) => print('IN: $l'));
  print('=== _fixSdp INPUT END ===');

  // 1. Detect dynamic LAN IP from c= line
  String? lanIp;
  final cLineRegex = RegExp(r'c=IN\s+IP[46]\s+([^\s\r\n]+)', caseSensitive: false);
  final m = cLineRegex.firstMatch(sdp);
  if (m != null) {
    final String ip = m.group(1)!;
    if (ip != '0.0.0.0' && ip != '127.0.0.1' && ip != wanIp) {
      lanIp = ip;
    }
  }
  ...
  final allowedPayloads = {'8', '0', '9', '101'};

  for (var line in lines) {
    // Dynamically allow all video payload types when the m=video line is encountered
    if (line.startsWith('m=video ')) {
      final parts = line.split(' ');
      if (parts.length > 3) {
        final videoPayloads = parts.sublist(3);
        allowedPayloads.addAll(videoPayloads);
      }
    }

    // Filter out unsupported rtpmap codecs
    if (line.startsWith('a=rtpmap:')) {
      final pm = RegExp(r'a=rtpmap:(\d+)').firstMatch(line);
      if (pm != null) {
        final payload = pm.group(1)!;
    ...
    // Filter out unsupported fmtp codecs
    if (line.startsWith('a=fmtp:')) {
      final pm = RegExp(r'a=fmtp:(\d+)').firstMatch(line);
      if (pm != null) {
        final payload = pm.group(1)!;
        if (!allowedPayloads.contains(payload)) {
          continue;
    ...
    // Rewrite m=audio line to only include allowed payloads
    if (line.startsWith('m=audio ')) {
      final parts = line.split(' ');
      if (parts.length > 3) {
        final prefix = parts.sublist(0, 3);
        final payloads = parts.sublist(3);
        final filteredPayloads = payloads.where((p) => allowedPayloads.contains(p)).toList();
        line = '${prefix.join(' ')} ${filteredPayloads.join(' ')}';
      }
    }

    // Skip candidate lines that contain IPv6 addresses to avoid duplicate candidate errors
    if (line.startsWith('a=candidate:')) {
      final parts = line.split(' ');
      if (parts.length > 4) {
        final ipPart = parts[4];
        if (ipPart.contains(':')) {
          continue;
        }
      }
    ...
    if (line.startsWith('a=connection:')) {
      continue;
    }

    // Replace IPs (IPv6 link-local, static IPv4 LAN, and dynamic LAN IP)
    line = line.replaceAll(RegExp(r'fe80::[0-9a-fA-F:]+'), wanIp);
    line = line.replaceAll('192.168.1.160', wanIp);
    if (lanIp != null) {
      line = line.replaceAll(lanIp, wanIp);
    }

    // Convert IP6 to IP4 in connection and origin lines
    line = line.replaceAll('IN IP6', 'IN IP4');

    // Lowercase fingerprint algorithm case-insensitively
    if (line.startsWith('a=fingerprint:')) {
      line = line.replaceAllMapped(RegExp(r'a=fingerprint:\s*([^\s]+)', caseSensitive: false), (match) {
        final algo = match.group(1)!.toLowerCase();
        return 'a=fingerprint:$algo';
      });
    }
  ...
  print('=== _fixSdp OUTPUT START ===');
  result.split('\n').forEach((l) => print('OUT: $l'));
  print('=== _fixSdp OUTPUT END ===');
...
class RTCSession extends EventManager implements Owner {
  ...
  void answer(Map<String, dynamic> options) async {
    ...
    logger.d('emit "sdp"');
    if (request.body != null) {
      print('=== RAW SDP OFFER START ===');
      request.body!.split('\n').forEach((l) => print('RAW: $l'));
      print('=== RAW SDP OFFER END ===');
    }
    final String? processedSDP = _sdpOfferToWebRTC(request.body);
    logger.d('processedSDP length: ${processedSDP?.length}');
    if (processedSDP != null) {
      print('=== PROCESSED SDP START ===');
      processedSDP.split('\n').forEach((l) => print('PROCESSED: $l'));
      print('=== PROCESSED SDP END ===');
    }
    if (processedSDP == null || processedSDP.isEmpty) {
      request.reply(488);
      _failed(Originator.system, null, null, null, 488,
          DartSIP_C.CausesType.WEBRTC_ERROR, 'SDP is null or empty after processing');
      return;
    }
    emit(EventSdp(
    ...
    print('=== DEBUG OFFER SDP LENGTH: ${processedSDP.length} ===');
    print('=== DEBUG OFFER TYPE: ${offer.type} ===');
    print('=== DEBUG OFFER TO_MAP: ${offer.toMap()} ===');
    try {
      await _connection!.setRemoteDescription(offer);
    } catch (error, stack) {
      print('=== SET_REMOTE_DESCRIPTION ERROR: $error ===');
      print(stack);
      request.reply(488);
      _failed(
          Originator.system,
        ...
        case SipMethod.ACK:
          ...
          if (_late_sdp) {
            ...
            RTCSessionDescription answer = RTCSessionDescription(
                _fixSdp(request.body, _ua.configuration.uri!.host), SdpType.answer.name);
            try {
  ...
  Future<RTCSessionDescription> _processInDialogSdpOffer(
    ...
    logger.d('emit "sdp"');
    logger.w('answer: request.body (raw):\n${request.body}');
    final String? processedSDP = _sdpOfferToWebRTC(request.body);
    logger.w('answer: processedSDP:\n$processedSDP');
    emit(EventSdp(
        originator: Originator.remote, type: SdpType.offer, sdp: processedSDP));

    if (processedSDP == null || processedSDP.trim().isEmpty) {
      request.reply(488);
      _failed(Originator.system, null, null, null, 488,
          DartSIP_C.CausesType.WEBRTC_ERROR, 'SDP is null or empty in answer');
      throw Exceptions.TypeError('SDP is null or empty in answer');
    }
  ...
  void _receiveInviteResponse(IncomingResponse? response) async {
    ...
    if (utils.test100(status_code)) {
      ...
      RTCSessionDescription answer =
          RTCSessionDescription(_fixSdp(response.body, _ua.configuration.uri!.host), SdpType.answer.name);
  ...
  String? _sdpOfferToWebRTC(String? sdpInput) {
    if (sdpInput == null) return null;
    return _fixSdp(sdpInput, _ua.configuration.uri!.host);
  }
